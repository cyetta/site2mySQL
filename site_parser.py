...
# return DataFrame if all ok, raise exception otherwise
# example: print_table(
...
def print_table(site_url, table_class_name):
    ...
    try:
        ...
        raise Exception(f'Some problem with the site "{site_url}"') from None

    soup = BeautifulSoup(response.text, 'html.parser')

    # Search given table  by tag 'class:'
    table = soup.find('table', attrs={'class': table_class_name})
    assert table , f'Table "{table_class_name}" not find'
